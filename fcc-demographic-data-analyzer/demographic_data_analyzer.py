...
def calculate_demographic_data(print_data=True):
    ...
    higher_education_rich = round((len(b) / len(a) * 100), 1)

    a = df.loc[~((df['education'] == 'Bachelors') | (df['education'] == 'Masters') | (df['education'] == 'Doctorate'))]
    b = a.loc[a['salary'] == '>50K']
    lower_education_rich = round((len(b) / len(a) * 100), 1)
    ...
    b = a.loc[a['salary'] == '>50K']
    rich_percentage = round(len(b) / len(a) * 100)

    # What country has the highest percentage of people that earn >50K?
    table = pd.crosstab(df['native-country'], df['salary'])
    table['percent'] = round((table['>50K'] / table[['<=50K', '>50K']].sum(axis=1)) * 100, 1)
    
    highest_earning_country = table.loc[table['percent'] == table['percent'].max()].index[0]
    
    highest_earning_country_percentage = table['percent'].max()
